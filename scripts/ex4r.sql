...
--
-- Required input:
-- - "ex4rLoc.r" R AMP Operations "mapping" script to install in database
-- - "ex4rGlb.r" R Global Average "reduce" script to install in database
...
CALL SYSUIF.REMOVE_FILE('ex4rLoc',1);
CALL SYSUIF.INSTALL_FILE('ex4rLoc','ex4rLoc.r','cz!/root/stoTests/ex4rLoc.r');

-- Register the script for global results across AMPs
CALL SYSUIF.REMOVE_FILE('ex4rGlb',1);
...
FROM SCRIPT(ON (SELECT CompanyID,
            ...
            HASH BY CompanyID
            SCRIPT_COMMAND ('Rscript --vanilla ./myDB/ex4rGlb.r')
